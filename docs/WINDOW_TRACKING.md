...
| 8 | `generic` | callerApp または cwd がある |
| 9 | `fallback` | 情報なし |
...
5. アプリ全体をアクティブ化（特定ウィンドウの選択なし）

ソケット保持プロセスの特定に `proc_listallpids` + `proc_pidfdinfo(PROC_PIDFDSOCKETINFO)` を使用する。
プロセスの PWD 取得に `sysctl(KERN_PROCARGS2)` を使用する。

### IntelliJ / JetBrains

cwd のフォルダ名マッチ → `__CFBundleIdentifier` からのプロジェクト名マッチ → 全IDE検索 → アプリをアクティブ化 の順で試みる。
...
フルスクリーン Space の場合は `app.activate()` で対応する。
...
- VSCode・IntelliJ・generic はウィンドウタイトルにプロジェクト名が含まれている必要がある
